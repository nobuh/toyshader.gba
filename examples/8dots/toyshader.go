...
func pseudoShader() {
	...
	for i := int16(0); i < 360; i += 360 / ncircle {
		x := orbit_radius * isin[i / 10] / 100 + o.x
		y := orbit_radius * icos[i / 10] / 100 + o.y
...
func main() {
	...
	for i := int16(0); i < 36; i++ {
		isin[i] = int16(100 * math.Sin(2.0 * 3.14 * float64(i) / 36))
		icos[i] = int16(100 * math.Cos(2.0 * 3.14 * float64(i) / 36))
	}
